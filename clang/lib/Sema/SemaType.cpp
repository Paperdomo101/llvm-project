...
TypeSourceInfo *Sema::GetTypeForDeclarator(Declarator &D) {
  ...
  if (getLangOpts().C4() && TInfo &&
      ...
      D.getDeclSpec().getTypeSpecType() != DeclSpec::TST_error) {
    bool IsParamRefLoc = false;
    if (D.isFunctionDeclarator()) {
      SourceLocation RefLoc = D.getDeclSpec().getC4ReferenceLoc();
      SourceLocation FuncLoc = D.getBeginLoc();
      if (RefLoc.isValid() && FuncLoc.isValid()) {
        if (Context.getSourceManager().isBeforeInTranslationUnit(FuncLoc, RefLoc)) {
          IsParamRefLoc = true;
        }
      }
    }
